...
/**
 * Redis 분산 락 테스트 - Redisson 직접 사용
 */
@SpringBootTest
class CouponServiceV5Test {

    @Autowired
    private CouponServiceV5 couponServiceV5;
    ...
    @Test
    @DisplayName("Redis 분산 락으로 동시성 제어 - Redisson 직접 사용 버전")
    void distributedLockTest() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try {
                    couponServiceV5.issueCoupon(couponId);
